 ...
 * Score components (weighted by tier). For Harvest: only applicable gates (max 45), then normalise to 0-100.
 * v2.3.2: Decomposed into content_score, schema_score, commercial_score for dashboard breakdown.
 */
class ReadinessScoreService
{
    /** Point value per component (full score = 100). */
    ...
    public function computeReadiness(Sku $sku): array
    {
        ...
        foreach (self::CHANNELS as $channel) {
            $channels[] = ['channel' => $channel, 'score' => $overall];
        }
    ...
    private function evaluateComponents(Sku $sku, bool $isHarvest): array
    {
        ...
        foreach (self::COMPONENTS as $key => $maxPoints) {
