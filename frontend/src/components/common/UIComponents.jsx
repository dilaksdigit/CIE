...
export const ReadinessBar = ({ value, width = 80 }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ width, height: 6, background: '#E8E8E8', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{
...
// ─── TREND LINE ─────────────────────────────────────
export const TrendLine = ({ data, width = 300, height = 60, color = '#5B7A3A' }) => {
...
export const ChannelBadge = ({ channel }) => {
    const map = { Amazon: '#8B6914', eBay: '#5B6B3A', Shopify: '#3D6B5B', Website: '#4A6B6B' };
    const bg = map[channel] || '#777';
    return (
        <span style={{
            display: 'inline-flex', padding: '2px 9px', borderRadius: 3,
            background: bg, color: '#FFF', fontSize: '0.6rem', fontWeight: 700,
